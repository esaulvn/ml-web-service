...
@app.post("/put_data_in_queue")
async def put_data_in_queue(data: dict):
    data_request = schemas.DataRequest(data=data)
    await data_queue.put(data_request)
    return {"message": "Data added to queue"}

@app.post("/predict_from_queue")
async def predict_from_queue(current_user: Annotated[schemas.User, Depends(get_current_active_user)], db: Session = Depends(get_db)):
    data_request = await data_queue.get()
    predict_result = predict(data_request=data_request, current_user = current_user, db = db)
    data_queue.task_done()
    return {"predict_result": predict_result}

@app.post("/predict")
def predict(data_request: schemas.DataRequest, current_user: Annotated[schemas.User, Depends(get_current_active_user)],
                  requested_model_type: str = 'logreg', db: Session = Depends(get_db)):
