...
async def fetch_predict_result(session, token):
    headers = {'Authorization': f'Bearer {token}'}
    async with session.post('http://127.0.0.1:8000/predict_from_queue', headers=headers) as response:
        if response.status == 200:
            data = await response.json()
            return data.get('predict_result')
        else:
...
async def main():
    async with aiohttp.ClientSession() as session:
        token = st.session_state.access_token
        predict_result = await fetch_predict_result(session, token)
        if predict_result:
            st.write('Ваш текст принадлежит к следующему классу:')
